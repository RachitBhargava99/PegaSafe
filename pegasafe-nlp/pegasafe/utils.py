...
_HTTP_HEADERS = {
    ...
    # "User-Agent": "pegasafe (Automatic text summarizer) Version/%s" % __version__,
}
...
def expand_resource_path(path):
    directory = dirname(sys.modules["pegasafe"].__file__)
...
def get_stop_words(language):
    ...
    try:
        stopwords_data = pkgutil.get_data("pegasafe", "data/stopwords/%s.txt" % language)
    except IOError as e:
